...
class BarCodeReader(object):
    ...
    def __init__(self, app):
        ...
        # do we need a second service ???
        self.bcr = session.service("ALBarcodeReader")
        self.bcr.subscribe("BarCodeReader") # we subscribe to face_detection as it is an extractor
    ...
    def on_bcr_detected(self, value):
        ...
        if value == []:  # empty value when the barcode disappears
            self.bcr_detected = False
        elif not self.got_barcode:  # only speak the first time a face appears
